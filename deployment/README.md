...
> **Status: Phase 9 complete - full observability surface live across all three services. `model_inference_duration_seconds` Histogram (single `model_name` label, 5 bounded values, 0.0001s-1.0s buckets) isolates per-model inference latency from request handling on every `/predict/*` and `/translate` call via a `track_inference` context manager. `input_distribution_sample` structlog event emits whole-vector summary stats (mean/std/min/max/L2/zero-fraction) every Nth request per PCA + DNN; cadence operator-tunable via `INPUT_DISTRIBUTION_SAMPLE_EVERY` env var (default 100, `0` disables). Five per-model `/health/<model>` endpoints (one per deployed model) return 200 with diagnostic body when loaded + recent, 503 with grep-friendly detail string (`model_not_loaded` or `inference_stale`) otherwise; freshness threshold operator-tunable via `MODEL_INFERENCE_STALENESS_SECONDS` env var (default 3600s). Full architectural rationale in `docs/monitoring.md` covering the four-golden-signals mapping + sample PromQL queries + drift-detection references (PSI, KS test; actual computation out of scope). 195/195 tests pristine across all 3 services; ruff + mypy strict clean. Moving to Phase 10 (OpenAPI/Swagger polish).** This README will be filled in at Phase 11 once all phases land.
