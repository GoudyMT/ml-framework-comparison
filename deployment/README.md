...
> **Status: all 12 phases complete; deployment phase shipped at v1.0.0.** Five models served across three FastAPI services (`sklearn-svc`, `pt-svc`, `tf-svc`) with MLflow-registry-backed loading, multi-stage Docker images, GitHub Actions CI/CD, complete observability surface (per-model inference-latency histogram + drift-signal log event + per-model freshness endpoints), exhaustive OpenAPI/Swagger metadata + `operation_id` on every endpoint, and a full operator + architecture documentation set. 195/195 tests pristine; ruff + mypy strict clean. See [`docs/architecture.md`](docs/architecture.md) for design rationale, [`docs/deployment-runbook.md`](docs/deployment-runbook.md) for the clone-to-running guide, [`docs/adding-a-new-model.md`](docs/adding-a-new-model.md) for the extension pattern, and the per-service READMEs at `services/<svc>/README.md` for operator how-tos.
...
- [x] **Phase 10 - OpenAPI/Swagger polish** (per-service FastAPI `contact` + `license_info` + `openapi_tags` metadata; `summary` + `description` + `responses` on every `/health` + `/predict` + `/metrics` endpoint; `model_config(json_schema_extra={"examples": [...]})` Request examples on all 5 schemas plus Response examples on DNN + Q-learning + translation; 1 real contract bug fixed (`n_output_tokens` description vs router behavior) + 3 cross-service consistency items addressed (duplicate `examples=`, GAN seed `None`, pt-svc `/health/<model>` description drift); containerized E2E smoke check 2026-05-19 verified Phase 9 + Phase 10 surface live in built images.)
- [x] **Phase 11 - Documentation** (full operator + architecture doc set landed: 3 per-service READMEs at `services/<svc>/README.md` covering boot + env vars + endpoint reference + troubleshooting per service; `docs/architecture.md` synthesizing design decisions across all services with explicit out-of-scope table; `docs/deployment-runbook.md` covering both the docker compose path and the host venv path with verification checklist + common failure modes; `docs/adding-a-new-model.md` walking through a 6th model across all 10 file touches; smoke-check observation carryover (monitoring.md HEALTHCHECK noise note, tf-svc translation schema example clarification); cross-doc consistency sweep tightened naming + scope discipline across all active markdown surface. OpenAPI hardening sub-phase added explicit `operation_id` on all 19 endpoints (clean SDK method names), promoted the private staleness-threshold accessor to public `get_staleness_threshold()`, and refactored pt-svc 3x `/health/<model>` blocks into a `_make_health_route` factory + `_HealthLoader` Protocol (~165 lines of triplication collapse to ~80 lines; the Protocol catches missing loader attributes at type-check time instead of at request time). 195/195 tests pristine across 3 services; ruff + mypy strict clean throughout.)
- [x] **Phase 12 - Git close-out** (mypy import-untyped suppressor added to each `[tool.mypy]` block to align local + CI behavior on stubless third-party imports (joblib, mlflow, tensorflow, sentencepiece); coordinated version bump 0.1.0 → 1.0.0 across all 3 services - `pyproject.toml [project]` version + `app/main.py FastAPI(version=)` per service; root README + this README status updated to reflect deployment-phase completion; annotated tag `deployment-phase-1.0.0` published. 195/195 tests pristine, ruff + mypy strict clean throughout.)
