...
WHAT THESE TESTS COVER:
    ...
    Integration through the router:
        - Hitting /predict/dnn N times via TestClient causes exactly
...
from app.schemas.dnn import INPUT_DIM
...
def test_helper_logs_on_nth_call() -> None:
    ...
    event = samples[0]
    assert event["model_name"] == "pt-dnn"
...
def test_helper_stats_correctness() -> None:
    ...
    with capture_logs() as captured:
        ...
        for _ in range(input_distribution.DEFAULT_SAMPLE_EVERY):
            input_distribution.maybe_log_input_distribution(
                features, model_name="pt-dnn-test"
            )
...
def test_router_emits_through_predict_endpoint(client: TestClient) -> None:
    """
    Hitting /predict/dnn DEFAULT_SAMPLE_EVERY times causes the helper to
    ...
    """
    payload = {"features": [0.5] * INPUT_DIM}
    with capture_logs() as captured:
        for _ in range(input_distribution.DEFAULT_SAMPLE_EVERY):
    ...
    assert samples[0]["model_name"] == "pt-dnn"
